...
for N = 1:n_iterations
    ...
    for current_psi_0 = psi_repeat_list
...
%          drawnow;
        ...
        for y = 1:length(states_out) % Test whether the trajectory crosses check radius
            if (norm([states_out(y,1) states_out(y,2)] - [1 1]) < check_radius)...
                ...
                for span_psi_0 = deg2rad(linspace(rad2deg(current_psi_0)-0.5/N,rad2deg(current_psi_0)+0.5/N,num_psi_0))
                    t         = t0;       % initialize t
                    step_size = check_radius/3;
                    n_steps   = tf/step_size;
                    tspan     = t0:step_size:tf;       % Total time span
                    currentStates = [pos_0 span_psi_0]';            % State array used inside the loop
    ...
    % check whether the solution has been found
