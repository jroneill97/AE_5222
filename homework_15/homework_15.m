...
int_guess        = 0.1*ones(1,25);
% int_guess(19:24) = (1e8)*ones(1,6);
% int_guess(20:22) = -int_guess(20:22);
% int_guess(25)    = 1e7;
options = optimoptions(@fmincon,"Algorithm","sqp","MaxFunctionEvaluations",20000);
states_out = fmincon(@cost_fun,int_guess,[],[],[],[],[],[],@homework_15_constraints,options);
...
fprintf("\n");
    ...
    function D_val = get_D(m,l)  
        ...
        if ~isequal(m,l)
            D_val = (legendreP(5,t(m))/legendreP(5,t(l)))*(1/(t(m)-t(l)));
        elseif (m==l) && (l==0)
            D_val = -(N*(N+1))/4;
        elseif (m==l) && (l==N+1)
            D_val = (N*(N+1))/4;
